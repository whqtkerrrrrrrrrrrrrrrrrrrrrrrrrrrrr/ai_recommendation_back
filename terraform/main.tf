...
resource "aws_db_parameter_group" "mysql_parameter_group_1" {
  name   = "${var.prefix}-mysql-parameter-group-1"
  family = "mysql8.0"
  ...
  tags = {
    Name = "${var.prefix}-mysql-parameter-group"
  }
}
...
resource "aws_db_instance" "db_1" {
  ...
  username                = "root"
  password                = "12341234"
  parameter_group_name    = aws_db_parameter_group.mysql_parameter_group_1.name
